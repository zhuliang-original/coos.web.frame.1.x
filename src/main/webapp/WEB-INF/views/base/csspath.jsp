<%@ page trimDirectiveWhitespaces="true"%><%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%><%@ page import="java.util.*"%><%@ include file="/WEB-INF/views/base/base.jsp"%>

<link href="${basePath}/resource/plugins/fontawesome/css/font-awesome.css" rel="stylesheet" media="screen">

<%-- <link href="${coos_css_path}" rel="stylesheet">
<link href="${coos_frame_css_path}" rel="stylesheet">
<link href="${coos_page_css_path}" rel="stylesheet"> --%>
<link rel="stylesheet/less" type="text/css" href="${coos_less_path}">
<link rel="stylesheet/less" type="text/css" href="${coos_frame_less_path}">
<link rel="stylesheet/less" type="text/css" href="${coos_page_less_path}">
<script type="application/javascript" src="${basePath }/resource/plugins/less/less.js"></script>